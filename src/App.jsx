...
export default function App() {
  ...
  const handleRequestOTP = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/login/phone/request-otp`, {
        phone: fullPhone
      });
      showToastMessage(response.data.message || `OTP sent to ${fullPhone}.`, 'success');
    } catch (error) {
      const errorMsg = error.response?.data?.message || 'Failed to send OTP.';
      showToastMessage(errorMsg, 'info'); // Using 'info' to match your existing CSS classes
    }
  };
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      showToastMessage(errorMsg, 'info');
    }
  };
  ...
  const handleEmailLoginSubmit = async (e) => {
    ...
    try {
      ...
      showToastMessage(`Welcome back! Logged in as ${response.data.name}`, 'success');
    } catch (error) {
  ...
  const handlePhoneLoginSubmit = async (e) => {
    ...
    try {
      const fullPhone = `${countryCode}${loginPhone}`;
      const response = await axios.post(`${API_BASE_URL}/login/phone/verify`, {
        phone: fullPhone,
        otp: loginOtp
      });
      ...
      showToastMessage(`Verified! Welcome back, ${response.data.name}`, 'success');
    } catch (error) {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      const fullPhone = regPhone.startsWith('+') ? regPhone : `${countryCode}${regPhone}`;
      ...
      const response = await axios.post(`${API_BASE_URL}/register`, {
        ...
        // Note: regRole is intentionally omitted from the backend payload based on previous DB schema, 
        // but kept in the UI state so the design doesn't break.
      });
      ...
      showToastMessage(`Account created for ${response.data.name} (${roleTitle})! Redirecting...`, 'success');
    } catch (error) {
