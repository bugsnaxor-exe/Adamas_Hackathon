...
exports.createRide = async (req, res) => {
  try {
    const { 
      ...
      travelDateTime, totalSeats, farePerSeat 
    } = req.body;
    ...
    res.status(500).json({ message: 'Error publishing ride', error: error.message });
  }
};
...
exports.searchRides = async (req, res) => {
  try {
    ...
    const { lng, lat, date, radiusInKm = 5 } = req.query;

    if (!lng || !lat || !date) {
      return res.status(400).json({ message: 'Please provide longitude, latitude, and date.' });
    }
    ...
    }).populate('driverId', 'name phone').populate('vehicleId', 'vehicleModel registrationNumber');

    res.status(200).json({ results: rides.length, rides });
  } catch (error) {
    res.status(500).json({ message: 'Error searching for rides', error: error.message });
  }
};
